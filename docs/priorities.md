...
| 10-19 | **Temporary Prohibitions** | Prohibitions with a start and stop date. |
| 20-29 | **Temporary Allowances** | Allowances with a start and stop date. |
...
| 40-49 | **User- and Time-specific Prohibitions** | Target one or more specific `user_class` or `purpose` and are also only in effect for specified `time_spans`. |
| 50-59 | **User- and Time-specific Allowances** | Target one or more specific `user_class` or `purpose` and are also only in effect for specified `time_spans`. |
| 60-69 | **User-specific Prohibitions** | Target one or more `user_class` or `purpose` but are not limited to specific `time_spans`. |
...
!!! warning "Restrictive Interpretation"
    Use of the restrictive interpretation may result in few or no instances of time- and/or 
    user-specific prohibitions. Interactions between allowances and prohibitions 
    has not been fully tested without the implicit exclusion assumption.